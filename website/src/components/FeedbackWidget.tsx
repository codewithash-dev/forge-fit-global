    ...
    <>
      ...
      {open && (
        <div className="fixed left-0 bottom-0 z-50 flex flex-row items-stretch max-w-[calc(100vw-2rem)]">
          {/* Dark card – left; rounded only on left side */}
          <div
            className="w-full max-w-md h-[70vh] min-h-[20rem] max-h-[28rem] bg-zinc-800 rounded-tl-2xl rounded-bl-2xl shadow-2xl border border-zinc-700 border-r-0 flex flex-col overflow-hidden shrink-0"
            role="dialog"
            aria-modal="true"
            aria-labelledby="feedback-title"
          >
            <div className="flex items-center justify-between shrink-0 border-b border-zinc-700 relative min-h-[3.5rem]">
              ...
              <div className="relative flex items-center gap-1.5 pl-5 py-3">
                <span className="text-white font-bold text-lg uppercase tracking-tight">ForgeFit</span>
              ...
              <button
                type="button"
                onClick={() => setOpen(false)}
                className="relative p-4 text-white hover:text-[#ea580c] text-xl font-bold leading-none"
                aria-label="Close"
              >
                ×
              </button>
            ...
            <div className="flex-1 overflow-y-auto p-6">
              <h2 id="feedback-title" className="text-white font-semibold text-base mb-4">
                ...
                {FEEDBACK_OPTIONS.map((option) => (
                  <li key={option}>
                    <label className="flex items-center justify-between gap-3 cursor-pointer group">
                      <span className="text-zinc-200 text-sm group-hover:text-white">{option}</span>
                      <input
                        type="radio"
                        ...
                        className="w-4 h-4 rounded-full border-2 border-zinc-600 bg-zinc-800 text-[#ea580c] focus:ring-[#ea580c] focus:ring-offset-0 accent-[#ea580c]"
                      />
                    </label>
                  </li>
                ))}
